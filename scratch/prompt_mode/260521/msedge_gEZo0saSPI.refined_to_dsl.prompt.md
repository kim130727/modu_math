# provider: openai
# model: gpt-5.4-mini
# image_path: examples\problems\260521\msedge_gEZo0saSPI.png
...
<refined_draft>
[문제 개요]
- 점 종이 위에서 4개의 점을 꼭짓점으로 하는 정사각형의 개수를 묻는 문제이다.
- 조건은 “가로, 세로로 이웃한 두 점 사이의 거리가 모두 같은 점 종이”이다.
- “크기가 같아도 위치가 다르면 다른 것으로 본다”는 셈 기준이 함께 제시되어 있다.
- 화면 구성은 위쪽의 문제 문장 + 아래쪽의 점 배열 그림으로 이루어진다.

[원본에 보이는 요소]
- 상단에 큰 글씨의 한국어 문제 문장이 여러 줄로 줄바꿈되어 있다.
- 하단에는 검은 점들이 일정 간격으로 배열된 그림이 있다.
- 점 외에 선분, 도형, 화살표, 표, 선택지, 답안칸은 보이지 않는다.
- 그림 안에는 숫자나 추가 문자는 보이지 않는다.
- 점들은 모두 동일한 크기의 작은 검은 원형 점으로 보인다.

[수식/관계 구조]
- 본문은 수식 전개식이 아니라 조건 설명형 문장이다.
- 핵심 관계는 다음과 같다.
  - 동일 간격의 점 배열
  - 4개의 점을 꼭짓점으로 하는 정사각형 탐색
  - 크기와 위치를 구분하여 개수 세기
- 하단 그림은 정사각형을 직접 그려 넣기 위한 배경 점 배열로 이해된다.
- 등호(`=`)를 포함한 계산식이나 변형식은 보이지 않는다.

[등호별 변환 의미]
- 해당 없음.
- 화면에 등호가 있는 식이 없으므로, 단계별 변환 설명도 없다.

[빈칸 정책]
- 원본에는 빈칸, 답칸, 채우기 칸이 보이지 않는다.
- 따라서 추론값을 넣거나 빈칸을 생성하지 않는다.
- 만약 DSL에서 답 영역이 필요하더라도, 이 원본 기준으로는 비워 둔 상태를 유지해야 한다.

[반복 패턴]
- 하단 점 배열은 규칙적으로 반복되는 격자형 패턴이다.
- 가로 방향과 세로 방향 모두 등간격으로 점이 놓여 있다.
- 눈에 보이는 범위에서는 대체로 4행으로 보이는 수평 줄이 확인된다.
- 각 행에는 여러 개의 점이 일정한 간격으로 반복된다.
- 전체적으로 정사각형 격자(등간격 점 배열)에 가까운 형태로 보인다.
- 좌우/상하 대칭에 가까운 인상이 있지만, 정확한 행·열 개수는 해상도상 확정하지 않는다.

[DSL 작성 힌트]
- 상단에는 문제 문장을 여러 줄 텍스트 박스로 배치하는 구성이 적절하다.
- 하단에는 점들만 있는 별도 그림 그룹을 두는 것이 원본과 잘 맞는다.
- 점 배열은 좌표 기반의 `dot grid` 스타일로 표현하는 것이 자연스럽다.
- 정사각형 자체는 그리지 말고, 점만 반복 배치된 배경으로 유지한다.
- 점들의 간격은 균등하게 유지하고, 행/열 정렬이 분명하게 보이도록 한다.
- 문장 줄바꿈은 화면에 보이는 배치에 맞추되, 의미 단위는 훼손하지 않는다.
- 답칸, 계산식 박스, 선택지는 추가하지 않는다.

[semantic/solvable 힌트]
- 이 문제는 점 배열 위에서 가능한 정사각형의 개수를 세는 유형으로 보인다.
- 단, 여기서는 개수를 계산하거나 정답을 추론하지 않는다.
- “크기가 같아도 위치가 다르면 다른 것으로 본다”는 조건은 중복 카운트 기준을 명시한다.
- 하단 점 배열이 정사각형을 만들 수 있는 기준 좌표 집합으로 사용된다.
- 실제 풀이용 의미는 있으나, 이 초안에서는 풀이 단계로 확장하지 않는다.

[주의 및 TODO]
- 점 배열의 정확한 행·열 개수는 해상도상 단정하지 말고 TODO로 남긴다.
- 하단 격자가 정확히 몇 행 몇 열인지 추가 확인이 필요하다.
- 가장자리 점이 잘려 보이는지 여부는 원본 확대 확인이 필요하다.
- 문제 문장 내 줄바꿈은 시각적 배치일 수 있으므로, 문장 의미와 구분해서 처리한다.
- 정사각형의 개수는 절대 추론하지 않는다.
- 원본에 정답이 인쇄되어 보이지 않으므로, 정답 관련 문구는 추가하지 않는다.
