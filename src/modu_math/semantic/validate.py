...
def _validate_answer(answer: dict[str, Any]) -> None:
    # Legacy semantic answer shape
    if "blanks" in answer or "choices" in answer or "answer_key" in answer:
    ...
    # New concise answer shape: {"type": "...", "value": ..., "unit": "..."}
    if "value" not in answer:
        raise SemanticValidationError(
            "answer must define either legacy keys (blanks/choices/answer_key) or answer.value"
        )
    answer_type = answer.get("type")
    if answer_type is not None and (not isinstance(answer_type, str) or not answer_type.strip()):
        raise SemanticValidationError("answer.type must be a non-empty string when present")
    if "unit" in answer and not isinstance(answer.get("unit"), str):
        raise SemanticValidationError("answer.unit must be a string when present")
