...
def validate_semantic_solvable_answer_match(
    semantic: dict[str, Any],
    solvable: dict[str, Any],
) -> None:
    semantic_answer = semantic.get("answer")
    solvable_answer = solvable.get("answer")

    if not isinstance(semantic_answer, dict):
        raise CrossLayerValidationError("$.semantic.answer must be an object")
    if not isinstance(solvable_answer, dict):
        raise CrossLayerValidationError("$.solvable.answer must be an object")
    def _normalize_answer(value: Any) -> Any:
        if isinstance(value, dict):
            ...
            for k, v in value.items():
                if k == "confidence":
                    continue
                out[k] = _normalize_answer(v)
            return out
        if isinstance(value, list):
            return [_normalize_answer(item) for item in value]
        return value

    if _normalize_answer(semantic_answer) != _normalize_answer(solvable_answer):
        raise CrossLayerValidationError(
            "semantic.answer must exactly match solvable.answer"
        )
