...
def _normalize_domain_for_generated_py(value: Any) -> dict[str, Any]:
    ...
    for idx, relation in enumerate(relations):
        ...
        from_id = relation.get("from_id", relation.get("from"))
        to_id = relation.get("to_id", relation.get("to"))
        if not isinstance(from_id, str) or not from_id.strip():
            continue
        if not isinstance(to_id, str) or not to_id.strip():
            # Invalid semantic relation for strict validator; skip it in generated DSL.
            continue

        relation_id = relation.get("id")
        if not isinstance(relation_id, str) or not relation_id.strip():
            relation_id = f"rel_{idx + 1}"
        relation_id = relation_id.strip()
        if relation_id in relation_ids:
            relation_id = f"{relation_id}_{idx + 1}"
...
def build_generated_py_template(
    *,
    ...
    domain = _normalize_domain_for_generated_py(semantic.get("domain", {}))
    ...
    lines = [
        ...
        f"    p.set_domain({_py_literal(domain)})",
        "    p.set_answer(",
    ...
    lines.extend(
        [
            ...
            "    # Running the exported module directly explicitly requests the optional SVG preview.",
            "    build().save(out_prefix, emit_svg=True)",
