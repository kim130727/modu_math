...
def _placement_sync_problem(tmp_path: Path, language: str, *, extra=True) -> Path:
    text = "안내" if language == "ko" else "Локалізована інструкція"
...
def test_placement_sync_preserves_ukrainian_translation(tmp_path: Path) -> None:
    client = _setup_django(tmp_path)
    paths = {lang: _placement_sync_problem(tmp_path, lang) for lang in ("ko", "uk")}
    for lang in paths:
        assert client.post(f"/api/editor/problems/{lang}/shared/build/").json()["ok"]

    ukrainian_before = json.loads(
        (paths["uk"] / "problem.layout.json").read_text(encoding="utf-8")
    )
    response = client.post(
        "/api/editor/problems/ko/shared/placement-sync/",
        data=json.dumps({
            "languages": ["uk"],
            "placements": [
                {"id": "slot.caption", "role": "question"},
                {"id": "slot.extra", "role": "choice"},
            ],
        }),
    ...
    assert client.post("/api/editor/problems/uk/shared/build/").json()["ok"]
    layout = json.loads(
        (paths["uk"] / "problem.layout.json").read_text(encoding="utf-8")
    )
    content = next(
        slot["content"] for slot in layout["slots"] if slot["id"] == "slot.caption"
    )
    original = next(
        slot["content"]
        for slot in ukrainian_before["slots"]
        if slot["id"] == "slot.caption"
    )
    assert content["semantic_role"] == "question"
    assert {k: v for k, v in content.items() if k != "semantic_role"} == {
        k: v for k, v in original.items() if k != "semantic_role"
    }
    assert not (paths["ko"] / "problem.editor_overrides.json").exists()


def test_placement_sync_rejects_unsupported_or_source_targets(tmp_path: Path) -> None:
    client = _setup_django(tmp_path)
    _placement_sync_problem(tmp_path, "ko")
    target = _placement_sync_problem(tmp_path, "uk")
    for languages, placements in [
        (["ko"], [{"id": "slot.caption", "role": "choice"}]),
        (["unsupported"], [{"id": "slot.caption", "role": "choice"}]),
        (["uk"], [{"id": "slot.caption", "role": "bad"}]),
        (["uk"], [{"id": "slot.not_present", "role": "choice"}]),
    ]:
        response = client.post(
            "/api/editor/problems/ko/shared/placement-sync/",
            data=json.dumps({"languages": languages, "placements": placements}),
            content_type="application/json",
        )
...
def test_answer_review_sync_preserves_ukrainian_choice_text(tmp_path: Path) -> None:
    ...
    save_answer_review("uk/shared", {
        **source,
        "note": "Українська нотатка",
        "choices": [
            {"id": "slot.extra", "label": "2", "text": "Додатково", "correct": True},
            {"id": "slot.caption", "label": "1", "text": "Локалізована інструкція", "correct": False},
        ],
    })

    response = client.post(
        "/api/editor/problems/ko/shared/answer-review-sync/",
        data=json.dumps({"languages": ["uk"]}),
        content_type="application/json",
    )
    ...
    assert client.post("/api/editor/problems/uk/shared/build/").json()["ok"]
    semantic = json.loads(
        (paths["uk"] / "problem.semantic.json").read_text(encoding="utf-8")
    )
    saved = semantic["answer"]["presentation"]["review"]
    assert [(choice["text"], choice["correct"]) for choice in saved["choices"]] == [
        ("Локалізована інструкція", True),
        ("Додатково", False),
    ]
    assert saved["note"] == "Українська нотатка"


def test_answer_review_sync_builds_missing_ukrainian_artifacts(tmp_path: Path) -> None:
    from modu_math_web.editor.services.answer_review import save_answer_review

    client = _setup_django(tmp_path)
    _placement_sync_problem(tmp_path, "ko")
    target = _placement_sync_problem(tmp_path, "uk")
    with (target / "problem.dsl.py").open("a", encoding="utf-8") as stream:
        stream.write(
            '\nSEMANTIC_OVERRIDE = {"answer": {"choices": '
            '[{"id": "c1", "text": "Альфа"}, {"id": "c2", "text": "Бета"}]}}\n'
        )
    save_answer_review("ko/shared", {
        "mode": "choice",
        "status": "verified",
        "note": "",
        "answers": [],
    ...
    assert response.json()["results"][0]["status"] == "success"
    semantic = json.loads(
        (target / "problem.semantic.json").read_text(encoding="utf-8")
    )
    assert semantic["answer"]["value"] == "Альфа"


def test_answer_review_sync_rejects_unrelated_language(tmp_path: Path) -> None:
    client = _setup_django(tmp_path)
    _placement_sync_problem(tmp_path, "ko")
    for languages in (["unsupported"], ["ko"], []):
        response = client.post(
            "/api/editor/problems/ko/shared/answer-review-sync/",
...
""".lstrip()
    ...
    body = response.json()
    assert body["ok"] is True
    layout = json.loads((problem_dir / "problem.layout.json").read_text())
    assert all(slot.get("id") != "slot.answer" for slot in layout["slots"])
    overrides = json.loads(overrides_path.read_text(encoding="utf-8"))
    assert overrides["deleted_slots"] == ["slot.answer"]
...
def test_layout_patch_converts_textslot_to_textboxslot_when_width_patched(
    tmp_path: Path,
) -> None:
    client = _setup_django(tmp_path)
    dsl_text = """
...
SLOTS = (
    TextSlot(id="slot.q1", text="Sample question", x=10.0, y=20.0, font_size=28),
)
""".lstrip()
    problem_dir = _write_problem(tmp_path, "0001", dsl_text)
...
def test_fast_layout_patch_adds_new_text_and_answer_slots_as_overrides(
    tmp_path: Path,
) -> None:
    client = _setup_django(tmp_path)
    dsl_text = """
...
PROBLEM_TEMPLATE = ProblemTemplate(
    id="p_fast_add",
    title="fast add",
    canvas=Canvas(width=500, height=240),
    regions=(
        Region(id="region.stem", role="stem", flow="absolute", slot_ids=("slot.q",)),
        Region(id="region.diagram", role="diagram", flow="absolute", slot_ids=()),
    ),
    slots=(TextSlot(id="slot.q", text="A", x=10, y=20, font_size=24),),
)
""".lstrip()
    problem_dir = _write_problem(tmp_path, "0001", dsl_text)
    ...
    assert response.status_code == 200
    assert (problem_dir / "problem.dsl.py").read_text(encoding="utf-8") == dsl_text
    overrides = json.loads(
        (problem_dir / "problem.editor_overrides.json").read_text(encoding="utf-8")
    )
    assert overrides["slots"]["konva_text_1"]["text"] == "보기"
    assert overrides["slots"]["konva_answer_1"]["interaction"]["role"] == "answer"
    assert overrides["slot_regions"] == {
        "konva_text_1": "region.diagram",
        "konva_answer_1": "region.diagram",
    }

    layout = {
        "regions": [
            {"id": "region.stem", "slot_ids": ["slot.q"]},
            {"id": "region.diagram", "slot_ids": []},
        ],
        "slots": [
            {
                "id": "slot.q",
                "kind": "text",
                "content": {"text": "A", "x": 10, "y": 20},
            }
        ],
        "reading_order": ["slot.q"],
    }
    applied = apply_editor_overrides(layout, overrides)
    slots = {slot["id"]: slot for slot in applied["slots"]}
    assert slots["konva_text_1"]["kind"] == "text_box"
    assert slots["konva_answer_1"]["kind"] == "rect"
    assert "konva_text_1" in applied["regions"][1]["slot_ids"]
    assert "konva_answer_1" in applied["regions"][1]["slot_ids"]
...
def test_apply_editor_overrides_line_geometry_removes_stale_transform() -> None:
    layout = {
        "canvas": {"width": 100, "height": 100},
        "slots": [
            {
    ...
    overrides = {
        "slots": {"slot.line": {"x1": 12.0, "y1": 22.0, "x2": 42.0, "y2": 52.0}}
    }
...
def test_prune_deleted_legacy_answer_slots_keeps_delete_without_visual_answer() -> None:
    layout = {
        "regions": [
            {
                "id": "region.stem",
    ...
    overrides = {"deleted_slots": ["slot.answer"], "version": 1}
    answer = {"value": 377, "answer_key": [{"slot_id": "slot.answer", "value": 377}]}
...
def test_prune_deleted_legacy_answer_slots_drops_blank_delete_with_visual_answer() -> (
    None
):
    layout = {
        "regions": [
...
def test_normalize_answer_for_deleted_slots_clears_empty_answer_lists() -> None:
    answer = {
        "value": 7,
        "blanks": [{"id": "slot.answer", "slot_id": "slot.answer", "expected": 7}],
        "answer_key": [{"slot_id": "slot.answer", "value": 7}],
    ...
    normalized, removed = normalize_answer_for_deleted_slots(answer, {"slot.answer"})
...
def test_build_accepts_choice_answer_key_ids_that_are_not_layout_slots(
    tmp_path: Path,
) -> None:
    client = _setup_django(tmp_path)
    dsl_text = """
...
PROBLEM_TEMPLATE = ProblemTemplate(
    ...
    canvas=Canvas(width=300, height=200),
    regions=(Region(id="region.stem", role="stem", slot_ids=("slot.q",)),),
    slots=(TextSlot(id="slot.q", text="Choose one", x=20, y=40),),
)

SEMANTIC_OVERRIDE = {
    "problem_id": "p_choice_answer",
...
SOLVABLE = {
    ...
    "plan": ["Choose the matching concept."],
    "steps": [{"id": "step.1", "expr": "diameter", "value": "지름"}],
    "checks": [{"id": "check.1", "expr": "diameter", "expected": True, "actual": True, "pass": True}],
    "answer": ANSWER,
}
""".lstrip()
    _write_problem(tmp_path, "0001", dsl_text)
...
def test_prune_editor_overrides_keeps_missing_slot_with_explicit_slot_region() -> None:
    layout = {
        "regions": [
            {"id": "region.stem", "slot_ids": ["slot.q"]},
            {"id": "region.answer", "role": "answer", "slot_ids": []},
        ],
        "slots": [
            {"id": "slot.q", "kind": "text", "content": {"text": "Prompt"}},
        ],
    }
    overrides = {
        "slots": {
        ...
        "slot_regions": {"konva_answer_1": "region.answer"},
        "version": 1,
    }
...
def test_prune_editor_overrides_keeps_missing_path_with_explicit_slot_region() -> None:
    layout = {
        "regions": [
            {"id": "region.stem", "slot_ids": ["slot.q"]},
            {"id": "region.diagram", "role": "diagram", "slot_ids": []},
        ],
        "slots": [
            {"id": "slot.q", "kind": "text", "content": {"text": "Prompt"}},
        ],
    ...
    overrides = {
        "slots": {
            "konva_arrow_1": {
                "d": "M 10 20 L 80 20 M 80 20 L 70 15 M 80 20 L 70 25",
                "fill": "none",
                "stroke": "#111827",
                "stroke_width": 1.2,
            }
        },
        "slot_regions": {"konva_arrow_1": "region.diagram"},
        "version": 1,
    }
