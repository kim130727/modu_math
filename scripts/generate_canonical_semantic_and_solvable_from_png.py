...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate canonical semantic JSON and solvable JSON from a PNG via OpenAI API."
        )
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help="Path to solvable schema (default: latest schema/solvable/solvable.v*.json)",
    )
    parser.add_argument(
        "--out-dir",
...
def _schema_version_key(tag: str) -> tuple[int, ...]:
    if not tag.startswith("v"):
        raise ValueError(f"Invalid schema tag: {tag}")
    return tuple(int(p) for p in tag[1:].split("."))


def find_latest_solvable_schema_path(repo_root: Path) -> Path:
    schema_dir = repo_root / "schema" / "solvable"
...
def extract_solvable_schema_value(solvable_schema: dict) -> str:
    try:
        schema_value = solvable_schema["properties"]["schema"]["const"]
    except Exception as exc:  # noqa: BLE001
        raise ValueError("Solvable schema must define properties.schema.const") from exc
    if not isinstance(schema_value, str) or not schema_value.startswith("modu.solvable.v"):
        raise ValueError(f"Invalid solvable schema const: {schema_value}")
    return schema_value
...
def generate_solvable(
    ...
    payload = generate_with_schema(
        ...
        user_instructions=(
            "Create solvable JSON from this PNG and semantic context. "
            f"Set `problem_id` to `{problem_id}` exactly. "
            f"Set `schema` to `{solvable_schema_value}` exactly. "
    ...
    payload["schema"] = solvable_schema_value
    payload["problem_id"] = problem_id
...
def main() -> None:
    ...
    solvable_schema_path = (
        Path(args.solvable_schema_file).resolve()
        if args.solvable_schema_file
        else find_latest_solvable_schema_path(repo_root)
    )
    ...
    solvable_schema_value = extract_solvable_schema_value(solvable_schema)
    ...
    solvable = generate_solvable(
        ...
        image_data_url=image_data_url,
        problem_id=args.problem_id,
        solvable_schema_value=solvable_schema_value,
    )
    ...
    solvable_tag = solvable_schema_value.replace("modu.solvable.", "", 1)
    solvable_out_path = out_dir / f"{prefix}.solvable.{solvable_tag}.json"
