...
PDF의 초등/중등 수학 문제를 추출하고, 렌더 가능한 semantic JSON으로 정규화한 뒤, Manim으로 정지형 문제 화면을 재생성하는 초기 코드베이스입니다.

## 프로젝트 목적

- `PDF -> semantic JSON -> Manim scene` 파이프라인 구축
...
3. Manim은 출력기이자 semantic JSON 검증기다.
4. 렌더 실패는 대개 schema 정보 부족 신호다.
5. 실패 로그를 schema 개선 근거로 남긴다.
6. 문제별 Manim 코드 누적 대신 공통 renderer + 유형별 semantic JSON으로 간다.
...
- `render coordinates`: Manim 장면 배치 좌표
